...
public class FileUtil {
     ...
     * example<p>
     * 传入参数：db，返回结果：/storage/emulated/0/Android/data/com.mtq.pndstatusmonitor/files/db
     * @param context
     * @return
     */
    public static File getExternalFilesDir(Context context, String childDirName){
        ...
        if (MEDIA_MOUNTED.equals(Environment.getExternalStorageState()) && hasExternalStoragePermission(context)) {
            returnFile = context.getExternalFilesDir(childDirName);
        }
        return returnFile;
    }
     ...
     * example<p>
     * 返回结果：/storage/emulated/0/log（log为传入的字符串参数）
     * @param childPath
     * @return
     */
